...
【⚠ 偏差表区域识别（最高优先级，必须首先判断）】
若本块中出现含 "DEVIATION TABLE"、"DEVIATION LIST"、"DIM NO."、"DEVIATION ACCEPTABLE"、
"FOR TOOL NO." 等表头文字的表格，则该表格内所有数值行必须按如下规则处理：
  - type 填 "deviation_override"（不得填 "dimension"）
  - dim_no 填表中该行的编号数字（如 39、40、11）
  - value 填偏差规格文本（如 "6.30+0.02/-0.04"、"0.50 D C"）
  - belongs_to_view 填 "DEVIATION_TABLE"
  - 绝对不要把偏差表中的数值当作正式尺寸（type="dimension"）识别
  - 若整块都是偏差表，views 写 ["DEVIATION_TABLE"]，elements 全部用 deviation_override
...
【严格只输出 JSON】不要解释、不要 markdown 代码块：
{
  ...
  "elements": [
    {
      "type": "dimension | gdt | thread | chamfer | radius | note | datum_label | angle | surface_roughness | datum_feature | deviation_override",
    ...
    def add(self, tile, result):
        ...
        for el in result.get("elements", []):
            ...
            if etype == "dimension":
                ...
                el["lower"] = calc["lower"]
                el["upper"] = calc["upper"]
            elif etype in ("angle", "deviation_override"):
                el["lower"] = None
                el["upper"] = None
...
class VisionExcelExporter(ExcelExporter):
    ...
    def export(self):
        ...
        deviation_overrides = m.by_type("deviation_override")
        ...
        # ── 覆盖 📋 偏差清单 sheet：融合文本层 + Vision 偏差表 ──
        # ExcelExporter.export() 已创建该 sheet（来自文本层解析）；
        # 若 Vision 识别到 deviation_override，追加写入同一 sheet。
        if deviation_overrides:
            dev_ws = self.wb["📋 偏差清单"]
            # 找到当前最后一行
            last_row = dev_ws.max_row + 1
            # 如果第一行是空提示行，则从第2行起重写（覆盖"未识别"提示）
            if last_row <= 2:
                # 表头已由 ExcelExporter 写好，直接追加数据
                pass
            for dov in deviation_overrides:
                dim_no_str = dov.get("dim_no", "")
                # 归一化编号格式为 "N#"
                if dim_no_str and not str(dim_no_str).endswith("#"):
                    dim_no_str = str(dim_no_str).strip() + "#"
                spec_val = dov.get("value", "") or dov.get("raw", "")
                for ci, val in enumerate([dim_no_str, "", spec_val], 1):
                    self._cell(dev_ws.cell(row=last_row, column=ci, value=val),
                               alt=(last_row % 2 == 0))
                last_row += 1
